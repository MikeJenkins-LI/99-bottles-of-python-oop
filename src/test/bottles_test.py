...
from src.bottles_of_beer.bottles import (
    BottleNumber,
    BottleNumber0,
    BottleNumber1,
    BottleNumber6,
...
def test_bottle_number_getter(number, expected) -> None:
    assert isinstance(BottleNumber.get(number), expected)


@pytest.mark.parametrize("number", [99, 98, 97, 8, 5, 3])
def test_verses_general_upper_and_lower_not_lasts(number: int) -> None:
    expected = (
...
class TestCombinedVerses:
    ...
    def test_a_few_verses(self) -> None:
...
"""
        ...
        assert expected == Bottles(TestCombinedVerses, 99, 0).verses(2, 0)
...
"""
        ...
        assert expected == Bottles(TestCombinedVerses, 99, 0).song()
